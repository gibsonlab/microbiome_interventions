---
title: "Gibson Mouse Forecast - modified from diet"
...
library(tidyverse)
library(glue)
#library(mdsine) we wont be using these
#library(fido2) we wont be using these
th <- theme_bw() +
  theme(
...
interventions <- read_csv("../_data/healthy/interventions.csv") |>
  column_to_rownames("sample")
reads <- read_csv("../_data/healthy/reads.csv") |>
  column_to_rownames("sample")
...
ts_missing <- subset_values(ts, 4)
ts_preds[["in-sample"]] <- predict(fits[["in-sample"]], ts_missing)


fits[["out-of-sample"]] <- mbtransfer(ts[c(1:3)], P = 2, Q = 2)
ts_preds[["out-of-sample"]] <- predict(fits[["out-of-sample"]], ts_missing[c(4)])
```
